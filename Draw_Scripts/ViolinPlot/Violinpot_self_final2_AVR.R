library(ggplot2)
library(ggsci)
data<- read.csv("C:\\Users\\22634\\Desktop\\result227merge1kg.csv",sep="\t",header=TRUE)
...
ggplot(data, aes(x = combined_pop, y = `length.Mb.`,fill=combined_pop)) +
  geom_violin(color="white",scale = "width")+
  stat_summary(fun=mean, geom="point", shape=20, size=3, color="black", fill="white", position=position_dodge(0.9)) +
  stat_summary(fun.y=mean, geom="text", aes(label=round(..y..,2), vjust=-4,hjust=0.4), position=position_dodge(0.9)) +
  ...
  ggtitle("Violinplot of Callsets in samples of 227 merge 1KGP")+
  theme_minimal()+
  theme(
...
ggsave("violin_plot2_avr.pdf", width = 16, height = 9, units = "in")
